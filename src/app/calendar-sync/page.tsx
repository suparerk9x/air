...
export default function SettingsPage() {
  ...
  // Group feeds by property — only show properties that have feeds
  const feedsByProperty = properties
    .map((prop) => ({
      property: prop,
      feeds: feeds.filter((f) => f.property.id === prop.id),
    }))
    .filter(({ feeds: f }) => f.length > 0);
              ...
              <div className="space-y-3">
                ...
                {feedsByProperty.map(({ property, feeds: propFeeds }) => {
                  ...
                  return (
                    <div
                      key={property.id}
                      className="bg-white rounded-lg border shadow-sm hover:shadow transition-shadow"
                    >
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="min-w-[160px]">
                                ...
                                <DropdownMenuItem className="text-red-600" onClick={() => handleDelete(propFeeds[0]?.id)}>
